...
export async function POST(request: Request, { params }: Params) {
  ...
  try {
    ...
    if (authorization.status !== "entered") {
      await client.query("rollback");
      return Response.json(
        { error: "Solo se puede registrar salida de visitantes ingresados" },
        { status: 409 },
      );
    }
    ...
    const existingExit = await client.query(
      "select id from access_events where authorization_id = $1 and event_type = 'exit' limit 1",
      [id],
    );

    if ((existingEntry.rowCount ?? 0) === 0) {
      await client.query("rollback");
      return Response.json({ error: "No existe entrada registrada" }, { status: 409 });
    }

    if ((existingExit.rowCount ?? 0) > 0) {
      await client.query("rollback");
      return Response.json({ error: "La salida ya fue registrada" }, { status: 409 });
    }

    await client.query(
      "update access_authorizations set status = 'completed', updated_at = now() where id = $1",
      [id],
    );
    ...
    const eventResult = await client.query(
      `
        ...
        values ($1, $2, $3, $4, $5, 'exit', 'exited', 'Salida registrada por porteria')
    ...
    return Response.json({
      ...
      message: `Salida registrada para ${authorization.visitor_name}.`,
    });
  } catch (error) {
    await client.query("rollback");
    throw error;
